...
pub fn rmsnorm_f32(
    ...
    if !x.is_contiguous() || !weight.is_contiguous() || !out.as_view().is_contiguous() {
        return Err(RmsNormError::ShapeMismatch);
    }
    ...
    for r in 0..rows {
        ...
        for i in 0..dim {
            let idx = r * dim + i;
            let v = unsafe { *x.get_flat_unchecked(idx) };
            mean_sq += v * v;
        }
        mean_sq /= dim as f32;
        let inv = 1.0f32 / sqrtf(mean_sq + eps);

        for i in 0..dim {
            let idx = r * dim + i;
            let v = unsafe { *x.get_flat_unchecked(idx) };
            let w = unsafe { *weight.get_flat_unchecked(i) };
            unsafe {
                *out.get_flat_unchecked_mut(idx) = v * inv * w;
            }
        }
    }
...
fn sqrtf(x: f32) -> f32 {
    ...
    {
        // Deterministic Newton-Raphson approximation.
        if x <= 0.0 {
            return 0.0;
        }
        let mut y = x;
