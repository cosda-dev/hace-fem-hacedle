...
pub fn rope_f32(
    ...
    if !x.is_contiguous() || !cos.is_contiguous() || !sin.is_contiguous() || !out.as_view().is_contiguous() {
        return Err(RopeError::ShapeMismatch);
    }
    ...
    for r in 0..rows {
        ...
        for i in 0..(last_dim / 2) {
            ...
            let x0 = unsafe { *x.get_flat_unchecked(idx0) };
            let x1 = unsafe { *x.get_flat_unchecked(idx1) };
            let c0 = unsafe { *cos.get_flat_unchecked(idx0) };
            let s0 = unsafe { *sin.get_flat_unchecked(idx0) };

            unsafe {
                *out.get_flat_unchecked_mut(idx0) = x0 * c0 - x1 * s0;
                *out.get_flat_unchecked_mut(idx1) = x1 * c0 + x0 * s0;
            }
        }
    }
