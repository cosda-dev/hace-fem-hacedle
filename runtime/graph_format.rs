...
impl<'a> GraphView<'a> {
    pub fn parse(data: &'a [u8]) -> Result<Self, GraphError> {
        ...
        let header = unsafe { core::ptr::read_unaligned(data.as_ptr() as *const GraphHeader) };
        ...
        if (base + node_start) % mem::align_of::<GraphNode>() != 0 {
            return Err(GraphError::Misaligned);
        }
        if (base + edge_start) % mem::align_of::<GraphEdge>() != 0 {
            return Err(GraphError::Misaligned);
        }
